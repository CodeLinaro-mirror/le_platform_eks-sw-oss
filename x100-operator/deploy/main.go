...
func init() {
    ...
    utilruntime.Must(qualcommv1.AddToScheme(scheme))
    utilruntime.Must(nfdk8s.AddToScheme(scheme))
    utilruntime.Must(opg.AddToScheme(scheme))
...
func main() {
    ...
    mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
        ...
        LeaderElectionID:       "49b9f7e5.qualcomm.com",
    ...
    healthInfo_gvk := schema.GroupVersionKind{
                      Group:   "",
                      Version: "v1",
                      Kind:    "Pod",
                      }
    restClient, err := apiutil.RESTClientForGVK(healthInfo_gvk, false, mgr.GetConfig(), serializer.NewCodecFactory(mgr.GetScheme()), &httpClient)
    if err != nil {
        setupLog.Error(err, "unable to create REST client", "for controller", "X100ManagementPolicy")
    }

    if err = (&controllers.X100ManagementPolicyReconciler{
